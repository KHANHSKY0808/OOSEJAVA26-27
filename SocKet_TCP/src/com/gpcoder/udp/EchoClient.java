...
public class EchoClient {
    ...
    public static void main(String[] args) {
        ...
        try {
            ds = new DatagramSocket(); // Tạo DatagramSocket cho Client
            System.out.println("Client started ");

            InetAddress server = InetAddress.getByName(SERVER_IP);
            while (true) {
                System.out.println("Enter your message: ");
                InputStreamReader isr = new InputStreamReader(System.in); // Nhập chuỗi từ bàn phím
                ...
                ds.send(dp); // Gửi gói tin sang Echo Server

                // Gói tin nhận
                DatagramPacket incoming = new DatagramPacket(BUFFER, BUFFER.length);
                ds.receive(incoming); // Chờ nhận dữ liệu từ EchoServer gửi về

                // Đổi dữ liệu nhận được từ mảng bytes ra chuỗi và in ra màn hình
